...
glm::vec3 modelPosition = glm::vec3(1.0f, 1.2f, -4.0f);
glm::vec3 modelPosition1 = glm::vec3(0.0f, 0.0f, -2.0f);
float rotationX = 0.0f;
float rotationY = 0.0f;
float rotationZ = 0.0f;

glm::vec3 cameraPos = glm::vec3(3.0f, 2.0f, 3.0f);
glm::vec3 cameraFront = glm::vec3(0.0f, 0.0f, 0.0f) - cameraPos;
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Mebel", NULL, NULL);
    if (window == NULL) {
    ...
    std::vector<Mesh> modelMeshes = loadModel("models/OBJ/Furniture Composition 75.obj");
    if (modelMeshes.empty()) {
        std::cerr << "Failed to load first model" << std::endl;
        return -1;
    }

    std::vector<Mesh> model1Meshes = loadModel("models/Untitled.obj");
    if (model1Meshes.empty()) {
        std::cerr << "Failed to load second model" << std::endl;
        return -1;
    }

    Mesh asphaltPlane = createPlane(10.0f, 10.0f);
    unsigned int asphaltTexture = loadTexture("models/velvre.jpg"); // Путь к текстуре асфальта
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Обновление матрицы вида на основе позиции камеры
        ...
        // Матрица модели для первой модели
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, modelPosition);
        model = glm::scale(model, glm::vec3(0.001f));
        ourShader.setMat4("model", model);

        // Отрисовка первой модели
        ...
        // Матрица модели для второй модели
        glm::mat4 model1 = glm::mat4(1.0f);
        model1 = glm::translate(model1, modelPosition1); // Позиция второй модели
        model1 = glm::scale(model1, glm::vec3(0.001f));
        ourShader.setMat4("model", model1);

        // Отрисовка второй модели
        for (const auto& mesh : model1Meshes) {
            glActiveTexture(GL_TEXTURE0);
            glBindTexture(GL_TEXTURE_2D, mesh.textureID);
            ourShader.setInt("texture1", 0);
